...
const list = async ({ req }, context, db) => {
    ...
    const properties = await getProperties(db, context, entity, view, whereParam, orderArray)
    const propertyList = []
...
const getProperties = async (db, context, entity, view, whereParam, orderArray = null) => {
    const listConfig = context.config[`${entity}/list/${view}`]
    const properties = [], propertyDefs = listConfig.properties
    const propertyList = Object.keys(propertyDefs)
