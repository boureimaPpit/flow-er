const renderForm = ({ context, entity, view} , { properties, formJwt }) => {
        ...
        for (let propertyId of section) {
            ...
            if (propertyId == "bank_identifier") {
                html.push(`<div class="form-group">
                    <input type="text" class="form-control property updateInput updateIban" name="${propertyId}" id="${propertyId}" value="${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
                </div>`)
            }
    
            else if (propertyType == "input") {
                html.push(`<div class="form-group">
                    <input type="text" class="form-control property updateInput" name="${propertyId}" id="${propertyId}" value="${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } ${ (readonly) ? "disabled" : "" } maxlength="255">
                </div>`)
            }
    
            else if (propertyType == "email") {
                html.push(`<div class="form-group">
                    <input type="text" class="form-control property updateInput updateEmail" name="${propertyId}" id="${propertyId}" value="${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
                </div>`)
            }
    
            else if (propertyType == "phone") {
                html.push(`<div class="form-group">
                    <input type="text" class="form-control property updateInput updatePhone" name="${propertyId}" id="${propertyId}" value="${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
                </div>`)              
            }
        
            else if (["date", "datetime", "closing_date"].includes(propertyType)) {
                html.push(`<div class="form-group">
                    <input type="text" class="form-control property updateInput updateDate" name="${propertyId}" id="${propertyId}" value="$${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
                </div>`)
            }
                ...
                html.push(`<div class="form-group">
                    ...
                    <select class="form-control property updateInput updateBirthYear" id="${propertyId}" ${(required) ? "required" : ""}>
                        <option />
                ...
                html.push(`<div class="form-group">
                <input type="text" class="form-control property updateInput updateTime" name="${propertyId}" id="${propertyId}" value="${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
            </div>`)              
            }
    
            else if (propertyType == "number") {
                html.push(`<div class="form-group">
                <input type="text" class="form-control property updateInput updateNumber" name="${propertyId}" id="${propertyId}" value="${value}" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
            </div>`)
            }
                ...
                html.push(`<div class="form-group row">
                    <label class="form-label">${(required) ? "* " : ""}${label}</label>
                    <textarea class="form-control property updateTextarea" rows="5" name="${propertyId}" id="${propertyId}" ${(required) ? "* " : ""}${label} maxlength="${(property.options.max_length) ? property.options.max_length : 2047}">${value}</textarea>
                </div>`)
            }
                ...
                html.push(`<div class="form-group" id="updateSelectDiv-${propertyId}">
                    <label class="form-label">${(required) ? "* " : ""}${label}</label>
                    <select class="form-control property updateSelect" name="${propertyId}" id="${propertyId}" ${(required) ? "required" : ""}>
                        <option />
    ${function () {
    ...
    ${function () {
        ...
        return html.join("\n")
                ...
                html.push(`<div class="form-group">
                    <input class="property updateCheckbox" type="checkbox" name="${propertyId}" id="${propertyId}" value="1" ${(required) ? "required" : ""} ></input>
                    &nbsp;&nbsp;
                    <label>${label}</label>
                </div>`)
            }
                ...
                html.push(`<div class="form-group">
                <input type="text" class="form-control property updateInput" name="${propertyId}" id="${propertyId}" value="" placeholder="${ (required) ? "* " : "" } ${label}" ${ (required) ? "required" : "" } maxlength="255">
            </div>`)
            }
