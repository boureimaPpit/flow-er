...
const renderUpdate = (context, entity, view, id, row, isDeletable, formJwt) => {
    ...
    const renderProperty = () => {
        ...
        for (let propertyId of Object.keys(properties)) {
            ...
            if (propertyType == "title") {
                ...
                html.push(`<div class="form-group row" id="updateSelectDiv-${propertyId}">
                    <label class="col-sm-5 col-form-label col-form-label-sm">${(mandatory) ? "* " : ""}${label}</label>
                    <div class="col-sm-7 selectTags" id="selectTags-${propertyId}"></div>
                </div>`)
            }
                ...
                if (property.source) {
                    const query = (property.source.query)? property.source.query : []
                    query[propertyId] = row[propertyId]
                }    
        ...
        return html.join("\n")
                ...
                if (property.source) {
                    const query = (property.source.query) ? property.source.query : []
                    query[propertyId] = (row[propertyId]) ? row[propertyId] : ""
                    html.push(`<input type="hidden" class="updateSelectedValue" id="updateSelectedValue-${propertyId}" value="${row[propertyId]}" />`)
                }
